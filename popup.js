...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const API_KEY = 'AIzaSyDgz2nLMQvwQfMFmPD7C9VqUCkBGPqkH1U';  // Replace with your actual YouTube Data API key
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (match && match[1]) {
      ...
      if (predictions) {
        ...
        let sentimentRating;
        let sentimentColor;
        if (normalizedSentimentScore >= 7) {
          sentimentRating = "Excellent to watch";
          sentimentColor = "lightgreen"; // Light Green
        } else if (normalizedSentimentScore >= 5) {
          sentimentRating = "Good to watch";
          sentimentColor = "orange"; // Light Yellow
        } else if (normalizedSentimentScore >= 3) {
          sentimentRating = "Average to watch";
          sentimentColor = "lightgrey"; // Light Orange
        } else {
          sentimentRating = "Wastage of time";
          sentimentColor = "lightred"; // Light Red
        }
        ...
        outputDiv.innerHTML += `
            ...
            <div class="metric" style="background-color: ${sentimentColor}; padding: 8px; border-radius: 5px; color: black; display: inline-block; text-align: center; width: 100%; font-weight: bold; font-size: 16px;">
  Sentiment Rating - ${sentimentRating}
</div>
  ...
  <div class="section">
    ...
    <ul class="comment-list">
      ${predictions.slice(0, 25).map((item, index) => {
          // Ensure the sentiment value is treated as a number
          const sentimentValue = parseInt(item.sentiment);

          // Assign background colors based on sentiment
          let bgColor, sentimentLabel;
          ...
          return `
          <li class="comment-item" style="background-color: ${bgColor}; color: white; padding: 10px; border-radius: 5px; margin-bottom: 5px;">
            <span><strong>${index + 1}.</strong> ${item.comment}</span><br>
            <span class="comment-sentiment" style="font-weight: bold;">Sentiment: ${sentimentLabel}</span>
          </li>`;
        }).join('')}
    </ul>
  </div>`;
      ...
      document.getElementById("download-btn").addEventListener("click", () => {
        const insights = `
          ...
          Avg Comment Length: ${avgWordLength} words
          Avg Sentiment Score: ${normalizedSentimentScore}/10
          Sentiment Rating: ${sentimentRating}
        `;
